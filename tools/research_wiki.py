#!/usr/bin/env python3
"""
ARIS Research Wiki — standalone and compatibility helper utilities.

The current research-lit workflow records note routes in ``wiki_notes.json``;
long-term wiki promotion is a separate explicit workflow and does not invoke
this script during the normal three-stage run.
...
def _arxiv_user_agent() -> str:
    ...
    verification helpers may use for the CrossRef polite pool — so no
...
def _ssl_context() -> ssl.SSLContext | None:
    ...
    no CA bundle configured, so every HTTPS request fails with
    CERTIFICATE_VERIFY_FAILED even though curl/browsers work fine (they use
    the system keychain instead). Mirrors the identical helper already added
    to arxiv_fetch.py / semantic_scholar_fetch.py / verify_papers.py — this
    was the one remaining tool in the project still missing it. Returns None
    (caller uses urllib's own default) if certifi isn't installed.
    """
    try:
        import certifi
    except ImportError:
