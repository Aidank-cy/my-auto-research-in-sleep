...
Called internally by ``research_candidate_search.py`` during current Stage 2.1;
downstream agents consume the resulting admission receipts instead of invoking
this helper independently.

Called directly at its fixed repo path (`tools/verify_papers.py`) — this
...
def should_cache_result(status: str) -> bool:
    """Cache stable verification outcomes only.

    `verify_pending` means a transient network/rate-limit failure blocked a
    layer, so persisting it would make future runs skip fresh verification.
    """
    return status in {"verified", "unverified", "error"}
...
def _ssl_context() -> ssl.SSLContext | None:
    ...
    the system keychain instead). Without this, every call in this module
    would fail identically and get misclassified by is_transient() as a
    retryable network blip, driving the retry/backoff/batch-split logic in
    _verify_arxiv_batch_with_retry to grind for a very long time without ever
    actually verifying anything. certifi ships its own trusted CA bundle;
    ...
    hard dependency. Mirrors arxiv_fetch.py's/semantic_scholar_fetch.py's own
    `_ssl_context()`.
    """
    try:
        import certifi
...
def polite_sleep(delay_seconds: float) -> None:
    """Sleep after outbound verifier requests when a positive delay is configured."""
    if delay_seconds > 0:
...
def verify_arxiv_batch(
    ...
    for i in range(0, len(ids), batch_size):
        ...
        result.update(_verify_arxiv_batch_with_retry(batch, delay_seconds))
    return result


def _verify_arxiv_batch_with_retry(
    batch: list[str],
    delay_seconds: float = DEFAULT_DELAY_SECONDS,
) -> dict[str, str]:
    ...
    if len(batch) > 1:
        mid = len(batch) // 2
        left = _verify_arxiv_batch_with_retry(batch[:mid], delay_seconds)
        right = _verify_arxiv_batch_with_retry(batch[mid:], delay_seconds)
        return {**left, **right}
...
def verify_doi(doi: str, user_email: str, delay_seconds: float = DEFAULT_DELAY_SECONDS) -> str:
...
def verify_title_s2(
    ...
    for base_url in (S2_API, S2_BULK_API):
        url = f"{base_url}?query={q}&limit=3&fields=title,year,externalIds"
        for attempt in range(2):
...
def verify_papers(
    ...
    *,
    ...
    # Layer 1: arXiv batch
    if to_verify_arxiv:
        arxiv_results = verify_arxiv_batch(
            list(to_verify_arxiv.keys()),
            arxiv_batch_size,
            delay_seconds,
        )
        for base_id, paper_ids in to_verify_arxiv.items():
            ...
            for pid in paper_ids:
                ...
                if cache is not None and should_cache_result(status):
    ...
    for p in to_verify_doi:
        status = verify_doi(p.doi or "", user_email, delay_seconds)
        result = PaperResult(
            id=p.id,
            status=status,
        ...
        if status == "unverified" and p.title:
            s2_status, s2_ids = verify_title_s2(p.title, fuzzy_threshold, delay_seconds)
        ...
        results[p.id] = result
        if cache is not None and should_cache_result(result.status):
    ...
    for p in to_verify_title:
        s2_status, s2_ids = verify_title_s2(p.title or "", fuzzy_threshold, delay_seconds)
        result = PaperResult(
            id=p.id,
            status=s2_status,
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="Seconds to sleep after each outbound verifier request (default: 2.0).",
    )
