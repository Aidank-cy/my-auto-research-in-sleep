...
# figure-extractor (the actual Huang-lab/figure-extractor clone, with its
# core/, app/, pdffigures2/ etc.) is a separate repo kept intact outside this
# project — see skills/paper-analysis/SKILL.md's FIGURE_EXTRACTOR_REPO input.
# core.extractor/core.metadata below live there, not in this project, so it
# must be on sys.path before the import.
sys.path.insert(0, "/Users/ninnnnk/Projects/figure-extractor")

# core.extractor hardcodes the bare command "java" (resolved via PATH at
# subprocess-call time, not configurable) to run pdffigures2.jar, which was
# built against Java 11 bytecode (compiled inside an isolated Docker image —
# see figure-extractor/Dockerfile — so no JDK toolchain was installed on the
# host to build it). The host's own default `java` is Java 8, too old for
# that jar. Rather than changing the system-wide default java (which would
# affect every other project), Java 11 was installed keg-only via
# `brew install openjdk@11` (Homebrew does not symlink keg-only formulas into
# PATH), and is prepended to PATH only for this process — and therefore only
# for the subprocess calls core.extractor makes within it — leaving the
# system-wide `java` on PATH for everything else untouched.
_JAVA11_BIN = "/opt/homebrew/opt/openjdk@11/bin"
if os.path.isdir(_JAVA11_BIN):
    os.environ["PATH"] = _JAVA11_BIN + os.pathsep + os.environ.get("PATH", "")
